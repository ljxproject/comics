...
class BaseWidget(forms.Form):
    ...
    def widget(self):
        if self.title == "历史公告":
            self.widget_icon = 'fa fa-bullhorn'
        elif self.title == "系统配置":
            self.widget_icon = "fa fa-server"
...
class Dashboard(CommAdminView):
    ...
    def get_widgets(self):
        ...
        if self.widget_customiz:
            ...
            portal_pos = UserSettings.objects.filter(
                user=1, key=self.get_portal_key())
            if len(portal_pos):
                ...
                if portal_pos:
                    ...
                    user_widgets = dict(
                        [(uw.id, uw) for uw in UserWidget.objects.filter(user=1, page_id=self.get_page_id())])
