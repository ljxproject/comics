...
class MangaUploadHandler(object):
    ...
    def manga_upload(self, filename):
        comics_resource_file = os.path.join(self.resources_path, filename)
        res = subprocess.Popen("python3 %s/load_comics_resource.py %s %s" %
                               (self.sdk_path, self.sdk_env_path, comics_resource_file),
                               shell=True,
                               stderr=subprocess.PIPE, encoding='utf-8').stderr.readlines()
